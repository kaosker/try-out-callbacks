...
AFTER YOU ARE FINISHED WITH THIS PROBLEM, ASK FOR A CODE REVIEW         <------------ ask for code review
...
let selectiveMap = function(array, cb1, cb2) {
    ...
    for (let i = 0; i < array.length; i++) {
        let el = array[i];
        if (cb1(el) === true) { // if callback1 is true, push results from callback2
            let result = cb2(el);
            newArr.push(result);
        } else {
            newArr.push(el); // else push the original element
        }
    }
    return newArr;
};
