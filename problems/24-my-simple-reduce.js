...
let mySimpleReduce = function(array, cb, initialAccum) {


    if (typeof initialAccum === "number") {
        array.unshift(initialAccum);
        let accum = array[0];    // first element === accum

        for (let i = 1; i < array.length; i++) {
            let currentValue = array[i];

            accum = cb(accum, currentValue, i, array)
        }
        return accum;
    } else {
        let accum = array[0];    // first element === accum
